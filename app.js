...
function render(){
 ...
 $('#stats').innerHTML=[['Sedute',DB.sessions.length],['Serie',sets],['Ripetizioni',reps],['Ultimo',fmtDate(latest?.date)]].map(x=>`<div class="stat"><b>${x[1]}</b><span class="muted">${x[0]}</span></div>`).join('');
 $('#latestTitle').textContent=latest.label; $('#latest').innerHTML=latest.exercises.map(exHTML).join('');
...
function renderNutrition(){
 const days=NUTRITION?.days||[], day=days[0], totals=day?.totals||sumDay(day);
 $('#nutritionDate').textContent=day?fmtDate(day.date):'—';
 const items=[['kcal',Math.round(totals.energy||0),'kcal'],['Proteine',Math.round(totals.protein||0),'g'],['Carboidrati',Math.round(totals.carbs||0),'g'],['Grassi',Math.round(totals.fat||0),'g']];
 ...
 $('#nutritionHistory').innerHTML=days.length?days.map(d=>{const t=d.totals||sumDay(d);return `<div class="session"><b>${fmtDate(d.date)}</b><div class="muted">${Math.round(t.energy||0)} kcal • P ${Math.round(t.protein||0)} g • C ${Math.round(t.carbs||0)} g • G ${Math.round(t.fat||0)} g • ${(d.meals||[]).length} pasti</div></div>`}).join(''):'<p class="muted">Il diario alimentare è pronto.</p>';
}
...
Promise.all([fetch('data/workouts.json?v='+Date.now()).then(r=>r.json()),fetch('data/nutrition.json?v='+Date.now()).then(r=>r.json())]).then(([w,n])=>{DB=w;NUTRITION=n;render();$('#search').addEventListener('input',e=>renderSessions(e.target.value));$('#exerciseSelect').addEventListener('change',e=>draw(e.target.value));$('#exportBtn').addEventListener('click',()=>{const a=document.createElement('a');a.href=URL.createObjectURL(new Blob([JSON.stringify({workouts:DB,nutrition:NUTRITION},null,2)],{type:'application/json'}));a.download='fitness-export.json';a.click()});window.addEventListener('resize',()=>draw($('#exerciseSelect').value))});